...
namespace hyppox {
    ...
    class Hyppox:public HInterface{
        ...
        Hyppox(){
            bool correct = true;
            if(typeid(std::string) == typeid(RowIDType)){
                std::cout<<"Row id: it shouldn't be a string. It should be either size_t or unsigned int."<<std::endl;
                correct = false;
            }
            
            if(typeid(std::string) == typeid(ClusterIDType)){
                std::cout<<"Cluster id: It shouldn't be a string. It should be either size_t or unsigned int."<<std::endl;
                correct = false;
            }
            
            if(typeid(std::string) == typeid(ClusterType)){
                std::cout<<"Cluster values: It shouldn't be a string. It should be either float or double."<<std::endl;
                correct = false;
            }
            
            if(typeid(std::string) == typeid(FilterType)){
                std::cout<<"Filter values: It shouldn't be a string. It should be either float or double."<<std::endl;
                correct = false;
            }
            
            // Forcefully terminalte the program
            if(!correct){
                exit(-1);
            }
        }
    ...
    template<typename RowIDType, typename ClusterIDType, typename Gentype, typename DatetimeType, typename ClusterType, typename FilterType>
    std::string Hyppox<RowIDType,ClusterIDType,Gentype,DatetimeType,ClusterType,FilterType>::getD3GraphObject(int argc, const char * argv[]){
        short genotypeCol ;
        std::vector<std::string> filterGen;
        std::vector<short> locationCol;
        std::vector<short> dateTimeCol;
    ...
    template<typename RowIDType, typename ClusterIDType, typename Gentype, typename DatetimeType, typename ClusterType, typename FilterType>
    void Hyppox<RowIDType,ClusterIDType,Gentype,DatetimeType,ClusterType,FilterType>::printHelpMessage(){
    ...
    template<typename RowIDType, typename ClusterIDType, typename Gentype, typename DatetimeType, typename ClusterType, typename FilterType>
    void Hyppox<RowIDType,ClusterIDType,Gentype,DatetimeType,ClusterType,FilterType>::parseIndex(std::string s, std::vector<short> &colIndices){
        colIndices.clear();
    ...
    template<typename RowIDType, typename ClusterIDType, typename Gentype, typename DatetimeType, typename ClusterType, typename FilterType>
    void Hyppox<RowIDType,ClusterIDType,Gentype,DatetimeType,ClusterType,FilterType>::parseFloat(std::string s, std::vector<float> &val){
        val.clear();
        s.erase( remove( s.begin(), s.end(), ' ' ), s.end() ); // Remove all spaces
    ...
    template<typename RowIDType, typename ClusterIDType, typename Gentype, typename DatetimeType, typename ClusterType, typename FilterType>
    int Hyppox<RowIDType,ClusterIDType,Gentype,DatetimeType,ClusterType,FilterType>::parseArgs(int argc, const char * argv[],
                           short &genotypeCol,
                           std::vector<std::string> &filterGen,
                           std::vector<short> &locationCol,
                           std::vector<short> &dateTimeCol,
    ...
    template<typename RowIDType, typename ClusterIDType, typename Gentype, typename DatetimeType, typename ClusterType, typename FilterType>
    std::string Hyppox<RowIDType,ClusterIDType,Gentype,DatetimeType,ClusterType,FilterType>::createMapper(){
    ...
    template<typename RowIDType, typename ClusterIDType, typename Gentype, typename DatetimeType, typename ClusterType, typename FilterType>
    std::string Hyppox<RowIDType,ClusterIDType,Gentype,DatetimeType,ClusterType,FilterType>::visualization(){
