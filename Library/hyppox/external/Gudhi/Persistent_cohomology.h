 ...
 public:
  ...
  explicit Persistent_cohomology(Complex_ds& cpx)
      : cpx_(&cpx),
        ...
        ds_repr_(num_simplices_, nullptr),                  // union-find -> annotation vectors
  ...
  void annotation_of_the_boundary(
    ...
    for (auto sh : cpx_->boundary_simplex_range(sigma)) {
      ...
      if (key != cpx_->null_key()) {  // A simplex with null_key is a killer, and have null annotation
        ...
        if (curr_col != nullptr) {  // and insert it in annotations_in_boundary with multyiplicative factor "sign".
...
// Update the cohomology groups:
    ...
    while (row_cell_it != death_key_row->second.row_->end()) {  // Traverse all cells in
      ...
      if (w != coeff_field_.additive_identity()) {
        ...
        if (curr_col->col_.empty()) {  // If the column is null
          ds_repr_[curr_col->class_key_] = nullptr;
 ...
 public:
  ...
  void output_diagram(std::ostream& ostream = std::cout) {
    ...
    ///////////////// This method created by Methun (methun@eecs.wsu.edu) on 3rd Feb, 2017 ///////////////
    std::vector<float> getPersistentFiltrationValue(short dim){
        ...
        for (auto pair : persistent_pairs_) {
            ...
            if (has_infinity && cpx_->filtration(get<1>(pair)) == std::numeric_limits<Filtration_value>::infinity()) {
                if((itr=maxD.find(cpx_->dimension(get<0>(pair))))==maxD.end()){
                    maxD.insert(std::pair<int, float>(cpx_->dimension(get<0>(pair)), cpx_->filtration(get<0>(pair))));
                }else{
                    if(itr->second < cpx_->filtration(get<0>(pair))){
                        itr->second = cpx_->filtration(get<0>(pair));
                    }
                }
                ...
                if((itr=maxD.find(cpx_->dimension(get<0>(pair))))==maxD.end()){
                    ...
                    if(itr->second < cpx_->filtration(get<1>(pair))){
                        itr->second = cpx_->filtration(get<1>(pair));
                    }
                }
        ...
        std::vector<float> v;
        for(itr=maxD.begin(); itr!=maxD.end(); itr++){
