...
void DBScan::ExpandCluster(unsigned long index, unsigned long clusterID){
    
    list<unsigned long> seeds;
...
void DBScan::GetSeeds(list<unsigned long> *phIndexList, unsigned long index){
    float value = this->phenotypeList[index]->getClusterValue();
    ...
    while (prevDiff <= this->Eps) {
        ...
        if(prevIndex >= 0){
            prevDiff = this->phenotypeList[prevIndex]->getClusterValue()-value;
    ...
    while (nextDiff <= this->Eps) {
        ...
        if(nextIndex < this->size){
            nextDiff = this->phenotypeList[nextIndex]->getClusterValue()-value;
