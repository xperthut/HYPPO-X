...
unordered_map<long, unordered_set<long> > UnionFindWithPathCompression::GetAllConnectedComponents(){
    
    this->Adjust();
    
    unordered_map<long, unordered_set<long> > scc;
    long index = 0;
    ...
    for(itr = treeRoots.begin(); itr != treeRoots.end(); itr++){
        ...
        scc.insert(pair<long, unordered_set<long> >(index, subGraphs));
        index++;
    }
