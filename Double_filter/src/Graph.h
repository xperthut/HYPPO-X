...
private:
    ...
    unordered_set<string> gen_loc_comb;
    list<int> dapList;
    
public:
    rNode(rNode* node);
    rNode(long id, float weight, float size);
    ~rNode();
    ...
    void setPhenotypeSummary(string genotype, string location, long plantID, int dap, float value);
...
private:
    ...
    float globalNodeWeightRange[2];
    float globalEnvWeightRange[FILTER][2];
...
public:
    ...
    void setGlobalRange(float nsRange[2], float nwRange[2], float ewRange[FILTER][2]);
...
public:
    ...
    void setRangeValues(CompositeGraph* c, float nsRange[2], float nwRange[2], float ewRange[FILTER][2], string ewName[FILTER]);
};
