...
rNode::rNode(long id, float weight, float size){
    this->id = id;
    this->weight = weight;
    this->size = size;
...
void rNode::setPhenotypeSummary(string genotype, string location, long plantID, int dap, float value){
    string key = genotype+"#"+location+"#"+to_string(dap)+"#"+to_string(plantID);
    
    this->gen_loc_comb.insert(genotype+","+location);
    dapList.push_back(dap);
...
string rNode::getGenLocComb(){
    string str = "";
    
    if(this->gen_loc_comb.size()<4){
        for(unordered_set<string>::iterator itr=this->gen_loc_comb.begin(); itr!=this->gen_loc_comb.end(); itr++){
            if(str.length()>0) str += "|";
            str += *itr;
        }
    }else{
...
float rNode::getAvgDap(){
    float sum = 0;
    for(list<int>::iterator itr=dapList.begin(); itr!=dapList.end(); itr++){
        sum += *itr;
    }
    
    if(dapList.size()>0) return sum/dapList.size();
...
bool rEdge::matchSignature(rEdge* e){
    ...
    if(INCREASE_DAP) return (getSignatureAtIndex(0) == e->getSignatureAtIndex(0) && getSignatureAtIndex(0)==1);
            ...
            for(int index=0; index<FILTER; index++){
                ...
                if(getSignatureAtIndex(index) != e->getSignatureAtIndex(index)){
                    ...
                    if(REFERENCE_PHENOTYPE){
                        ...
                        wDiff = (abs(getEdgeWeight(index+1)-e->getEdgeWeight(index+1))*100)/((getEdgeWeight(index+1)-e->getEdgeWeight(index+1))>0?getEdgeWeight(index+1):e->getEdgeWeight(index+1));
                        ...
                        if(index==0){
                            ...
                            wDiff = (abs(getEdgeWeight(index)-e->getEdgeWeight(index))*100)/((getEdgeWeight(index)-e->getEdgeWeight(index))>0?getEdgeWeight(index):e->getEdgeWeight(index));
...
void CompositeGraph::setCCNumber(int ccNumber){this->CCNumber = ccNumber;}

float CompositeGraph::adjustSize(float size){
    
    float max = NODE_SIZE_MAX, min = NODE_SIZE_MIN;
...
void CompositeGraph::setGlobalRange(float *nsRange, float *nwRange, float (*ewRange)[2]){
    globalNodeSizeRange[0] = nsRange[0];
    globalNodeSizeRange[1] = nsRange[1];
    
    globalNodeWeightRange[0] = nwRange[0];
    globalNodeWeightRange[1] = nwRange[1];
    
    for(int i=0; i<FILTER; i++){
        globalEnvWeightRange[i][0]=ewRange[i][0];
        globalEnvWeightRange[i][1]=ewRange[i][1];
    }
}
...
void CompositeGraph::getWidthAndColor(long fromID, long toID, string *property){
    ...
    for(int i=0,c=0; i<pathCounter; i++){
        ...
        if(p->getPath().size() >= MINIMUM_PATH_LENGTH){
            ...
            if(PRINT_ALL_PATHS){
                if(p->hasNodes(id1)||p->hasNodes(id2)){
                    property[0] = to_string(EDGE_WIDTH);
                ...
                int pathIds[]=PATH_LIST;
                string manual_colors[]=PATH_COLOR;
                
                // total connected components
                for(int j=1; j<=pathIds[0]; j++){
                    counter++;
                    
                    if(pathIds[counter] == this->CCNumber){
                        counter++;
                        int limit = pathIds[counter];
                        
                        // Loop until all paths
                        for(int k=1;k<=limit;k++){
                            counter++;
                            
                            if(c==pathIds[counter]){
                                if(p->hasNodes(id1)||p->hasNodes(id2)){
                                    property[0] = to_string(EDGE_WIDTH);
                                    
                                    if(ASSIGN_PATH_COLOR_MANUAL){
                                        property[1] = manual_colors[counter];
                                    }else{
                                        property[1] = p->getColor();
                                    }
...
string CompositeGraph::printGraph(bool isPhenotype, bool isEnv, int index, string fileNameSuffix){
    ...
    if(HEATMAP_EACH_CC)
        c.SetRange(nodeWeightRange[0], nodeWeightRange[1]);
    else
       c.SetRange(globalNodeWeightRange[0], globalNodeWeightRange[1]);
    ...
    for(nitr=nList.begin(); nitr!=nList.end(); nitr++){
        ...
        if(isPhenotype){
            ...
            if(HEATMAP_EACH_CC)
                ce.SetRange(envWeightRange[index][0], envWeightRange[index][1]);
            else
                ce.SetRange(globalEnvWeightRange[index][0], globalEnvWeightRange[index][1]);
...
void CompositeGraph::reconstructPaths(){
    ...
    for(list<rEdge*>::iterator itr=tmpEdgeList.begin(); itr!=tmpEdgeList.end(); itr++){
        ...
        if(exploreEdge(e)){
            ...
            Path* p = interestingPaths.getValue(pathCounter);
            unordered_set<string> pKey = p->getPath();
            
            for(unordered_set<string>::iterator pItr=pKey.begin(); pItr!=pKey.end(); pItr++){
                string s = *pItr;
                //string key = to_string(fromNode->getID())+"#"+to_string(toNode->getID());
                rEdge* e = edgeMap.getValue(s);
    ...
    if(!ASSIGN_PATH_COLOR_MANUAL) assignPathColor();
}
...
void CompositeGraph::absoluteDistanceWeight(){
    ...
    for(unordered_map<string, rEdge*>::iterator itr=eList.begin(); itr!=eList.end(); itr++){
        ...
        /*if(fromNode->getID()==62 && (toNode->getID()==81)){
            cout<<"";
        }*/
...
string CompositeGraph::printCompositeGraph(bool withSignature, string fileNameSuffix){
    ...
    for(nitr=nList.begin(); nitr!=nList.end(); nitr++){
        ...
        //string label = to_string(n->getID()) + "[" + n->getGenLocComb() + "]{" + fixPrecision(n->getWeight(), 2) + "}";
        string shape = "pie";
...
string CompositeGraph::printCompositeGraphsWithPath(string fileNameSuffix){
    ...
    this->constructCompositeGraph();
    
    if(GENERATE_PATHS){
        if(EXPLORE_PHENOTYPE_GRAPH)
            this->runBFSonPhenotype();
        else
            this->runBFS();
    ...
    for(list<Path*>::iterator itr=pathList.begin();itr!=pathList.end(); itr++){
        ...
        for(list<long>::iterator nItr=nList.begin(); nItr!=nList.end(); nItr++){
            ...
            for(unordered_map<string, list<float>>::iterator phItr=phSummary.begin(); phItr!=phSummary.end(); phItr++){
                ...
                key = genotype + "$" + location + "$" + plantID;
                
                AllInd.insert(key);
                tmp.insert(key);
            }
            
            nodeIndv.insert(pair<long, unordered_set<string>>(nID, tmp));
        }
...
string CompositeGraph::analyzeNodes(){
    ...
    for(unordered_map<long, rNode*>::iterator nItr=nList.begin(); nItr!=nList.end(); nItr++){
        ...
        for(unordered_map<string, list<float>>::iterator phItr=phSummary.begin(); phItr!=phSummary.end(); phItr++){
            ...
            csv += genotype + "," + location + "," + plantID + "," + dap + "," + to_string(n) + "," + pCol + "," + getGenLocColor(genotype, location) + "," + fixPrecision(getMeanValue(phValList), 2) + "\n";
...
MainGraph::MainGraph(int ccCounter){
    ...
    for(int i=1; i<=ccCounter; i++){
        this->connectedComponents[i-1].setCCNumber(i);
    }
    
    globalNodeSizeRange[0]=globalNodeSizeRange[1]=0;
    globalNodeWeightRange[0]=globalNodeWeightRange[1]=0;
    for(int i=0; i<FILTER; i++){
        globalEnvWeightRange[i][0]=globalEnvWeightRange[i][1]=0;
    }
...
CompositeGraph* MainGraph::getCC(int index){
    if(index>=0&&index<ccCounter){
        return &this->connectedComponents[index];
    }
...
void MainGraph::printOriginalGraph(string fileNameSuffix){
    ...
    if(PRINT_BARCODE) gmlData = "graph\n[\ndirected 0\nweighted 1\n";
...
void MainGraph::printAllGraphs(string fileNameSuffix){
    ...
    for(int j=0;j<ccCounter; j++){
        connectedComponents[j].setGlobalRange(globalNodeSizeRange, globalNodeWeightRange, globalEnvWeightRange);
    }
...
void MainGraph::PrintAllNodeStatistics(string fileNameSuffix){
    string csv = "Genotype,Location,PlantID,DAP,NodeID,plant.id.color,Loc.Gen.color,Growth.rate\n";
...
void MainGraph::setRangeValues(CompositeGraph *c, float *nsRange, float *nwRange, float (*ewRange)[2], string *ewName){
    c->storeRange(nsRange, nwRange, ewRange, ewName);
    ...
    if(first){
        ...
        if(globalNodeSizeRange[0]>nsRange[0])globalNodeSizeRange[0]=nsRange[0];
        if(globalNodeSizeRange[1]<nsRange[1])globalNodeSizeRange[1]=nsRange[1];
        
        if(globalNodeWeightRange[0]>nwRange[0])globalNodeWeightRange[0]=nwRange[0];
        if(globalNodeWeightRange[1]<nwRange[1])globalNodeWeightRange[1]=nwRange[1];
        
        for(int i=0; i<FILTER; i++){
            if(globalEnvWeightRange[i][0]>ewRange[i][0])globalEnvWeightRange[i][0]=ewRange[i][0];
            if(globalEnvWeightRange[i][1]<ewRange[i][1])globalEnvWeightRange[i][1]=ewRange[i][1];
        }
    }
}
