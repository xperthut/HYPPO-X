...
private:
    ...
    void getIndividualsOfRegion(list<Phenotype*>* clPhList, float &x1, float &x2, float &min, float &max);
...
public:
    ClusteringRegion(list<DataPoint*>* dataPointList, float xMin, float yMin, float xMax, float yMax, float wSizeX, float wSizeY, float overlapX, float overlapY);
    ~ClusteringRegion();
    ...
    void GenerateSimplicialComplex(SimplicialComplex* sc);
};
