Canvas
======
The `Javascript` object named `canvas` of `HTML 5` is used here to represent graph more meaningfully using `pie chart` and color. 
...
### Output
Run the `HTML` file in any browser. One can save the image in a `png` formatted file.
