...
### Input
The output of our graph plotting project in `R` is an `eps` formatted image file. We changed the format of this image from `eps`  to `png`, which is used as a background image of this project. We plotted a pie chart on a node position of the background image. Our `C++` code generates `JSON` data for both `pie chart` and `node coloring` by enabling the flag named `PIE_CHART_CODE` in `config.h` file.
